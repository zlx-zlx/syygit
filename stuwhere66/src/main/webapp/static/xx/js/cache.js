...
layui.use(['form','jquery',"layer"],function() {
    ...
    if(window.sessionStorage.getItem('userFace') &&  $(".userAvatar").length > 0){
        ...
        $("#userFace").attr("src","../../images/mayun.png");
    }
  ...
  /*//公告层
    function showNotice(){
        layer.open({
            ...
            btn: ['知道了'],
            moveType: 1,
            content: '<div style="padding:15px 20px; text-align:justify; line-height: 22px; text-indent:2em;border-bottom:1px solid #e2e2e2;"><p class="layui-red">请使用系统前请务必仔细阅读左上角公告，避免使用中遇到一些简单的问题造成困扰。</p></pclass></p><p>1.0发布以后发现很多朋友将代码上传到各种素材网站，当然这样帮我宣传我谢谢大家，但是有部分朋友上传到素材网站后将下载分值设置的相对较高，需要朋友们充钱才能下载。本人发现后通过和站长、网站管理员联系以后将分值调整为不需要充值才能下载或者直接免费下载。</div>',
            success: function(layero){
                var btn = layero.find('.layui-layer-btn');
                btn.css('text-align', 'center');
                btn.on("click",function(){
    ...
    function lockPage(){
        layer.open({
            ...
            content : '<div class="admin-header-lock" id="lock-box">'+
                            '<div class="admin-header-lock-img"><img src="static/images/mayun.png" class="userAvatar"/></div>'+
                            '<div class="admin-header-lock-name" id="lockUserName">weej1</div>'+
                            '<div class="input_btn">'+
