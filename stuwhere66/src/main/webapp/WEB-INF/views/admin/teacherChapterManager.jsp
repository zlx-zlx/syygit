...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span>
	 	<a href="<%=path%>/chapter/showCourse">学校管理</a> <span class="c-gray en">&gt;</span><a href="javascript:location.replace(location.href);">学校信息</a> 
		...
		<tbody id="students">
			...
			<c:forEach items="${students }" var="student"  varStatus="sta">
				<tr class="text-c">
					...
					<td><input class="button button-little bg-red" type="button"
								onclick="admin/chapterDelete(${student.stuId},${schCode })" value="删除"></td>
					...
					<td class="td-manage">
						...
						<%-- <a title="删除" href="<%=basePath%>admin/paperDelete?stuId=${student.stuId}"  onclick="return confirm('是否删除？')" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
							 --%>
							<a href="<%=basePath%>admin/removequestion?stuId=${student.stuId}&schCode=${student.schCode}" onclick="return confirm('是否移除？')">移除</a>
						
						<a title="删除" href="<%=basePath%>admin/paperDelete?stuId=${student.stuId}" class="ml-5" style="text-decoration: none"><i class="Hui-iconfont">&#xe6e2;</i></a>
						
						
						<%-- <a title="删除" href="javascript:;" onclick="member_dell(this,'${student.stuId }')" class="ml-5" style="text-decoration:none"><i class="Hui-iconfont">&#xe6e2;</i></a>
					
					 --%>
					
					<%-- <a title="删除" class="button button-little bg-red" type="button"
								 href="<%=basePath%>admin/deleteChapter?stuId=${student.stuId}&schCode=${student.schCode })" onclick="return confirm('是否移除？')" value="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
						 --%>
					
					<%-- <a title="删除" class="button button-little bg-red" type="button"
								 href="<%=basePath%>admin/chapterDelete?stuId=${student.stuId}&schCode=${student.schCode})" onclick="return confirm('是否移除？')" value="删除"><i class="Hui-iconfont">&#xe6e2;</i></a>
						 --%>
...
function member_del(obj, stuId) {
	layer.confirm('确认要删除吗？', function(index) {
		...
		$.ajax({
			...
			url: 'admin/paperDelete?stuId='+stuId,
			dataType: 'json',
			success: function(data){
				if(data==1){
					
					layer.msg('已删除!',{icon:1,time:1000});
				   //location.href="admin/chapterShowTable34";
			}
				else{
...
function member_dell(obj, stuId) {
	layer.confirm('确认要删除dd吗？', function(index) {
		$(obj).parents("tr").remove();
		$.ajax({
			type: 'POST',
			...
			success: function(data){
				if(data==1){
				   ...
				   // location.href="admin/chapterManager";
			}
				else{
					layer.msg('删除失败！',{icon:1,time:4000});
					location.href="admin/chapterManager";
				}
			},
			error:function(data) {
